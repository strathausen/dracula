...
Raphael.fn.connection = function Connection(obj1, obj2, style) {
  ...
  let edge = {/*
    ...
    draw() {
      ...
      let res = dis.length === 0 ? [0, 4] : d[Math.min.apply(Math, dis).toFixed(3)];
      ...
      let path = ['M' + x1.toFixed(3), y1.toFixed(3),
      ...
      if (style && style.directed) {
        ...
        let norm = function(x, l) { return (-x*(l||5)/mag); };
      ...
      style && style.label &&
          (edge.label && edge.label.attr({x: (x1+x4)/2, y: (y1+y4)/2}) ||
              (edge.label = selfRef.text((x1+x4)/2, (y1+y4)/2, style.label)
                .attr({fill: '#000', 'font-size': style['font-size'] || '10px', 'fill-opacity': '0.6'})));
